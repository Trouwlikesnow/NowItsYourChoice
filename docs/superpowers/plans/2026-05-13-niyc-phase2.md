...
def test_portfolio_schema_well_formed_and_links_to_sectors():
    ...
    names = {f.name for f in schema}
    assert {"股票代码", "股票名称", "所属板块", "持仓数量", "成本价",
            "成本金额", "当前价", "市值", "浮盈额", "浮盈%",
...
def test_asset_snapshots_schema_well_formed():
    ...
    _assert_well_formed(schema)
    names = {f.name for f in schema}
    assert {"日期", "总市值", "总成本", "总浮盈", "总浮盈%",
            "当日盈亏", "持仓只数"}.issubset(names)
```
...
class TestSyncPortfolio:
    def test_buy_creates_new_position(self):
        bitable = MagicMock()
        # One unprocessed trade: buy 002594 1000 @ 10
        bitable.list_records.side_effect = [
            ...
            # portfolio query for 002594
            [],
        ]

        sync_portfolio(_cfg(), bitable)

        # Should create a new position record
...
def sync_portfolio(cfg, bitable) -> None:
    ...
    for code, code_trades in by_code.items():
        ...
        positions = bitable.list_records(
            ...
            filter_=f'CurrentValue.[股票代码] = "{code}"',
        )
        existing = None
        pos_record_id = None
        if positions:
        ...
        if pos_record_id:
            ...
            record = {"股票代码": code, "股票名称": name, **existing}
...
class TestTakeAssetSnapshot:
    def test_creates_snapshot_row(self, mocker):
        bitable = MagicMock()
        bitable.list_records.side_effect = [
            # portfolio
            [
            ...
            [
                {"record_id": "snap_y", "fields": {"总市值": 19000}},
            ],
        ]
        ...
        row = args[1][0]
        assert row["总市值"] == 20000
        assert row["总成本"] == 17530
        assert row["总浮盈"] == pytest.approx(2470)
        assert row["当日盈亏"] == pytest.approx(1000)
        assert row["持仓只数"] == 2
```
...
def take_asset_snapshot(cfg, bitable) -> None:
    """Generate a daily asset snapshot row."""
    positions = bitable.list_records(cfg.tables.portfolio)
    active = [p for p in positions if _num(p["fields"].get("持仓数量")) > 0]

    total_mv = sum(_num(p["fields"].get("市值")) for p in active)
    total_cost = sum(_num(p["fields"].get("成本金额")) for p in active)
    total_profit = round(total_mv - total_cost, 2)
    total_profit_pct = round(total_profit / total_cost * 100, 2) if total_cost else 0.0

    # Get yesterday's snapshot for daily P&L
    yesterday_snaps = bitable.list_records(cfg.tables.asset_snapshots)
    yesterday_mv = 0.0
    if yesterday_snaps:
        # Take the last one (most recent)
        yesterday_mv = _num(yesterday_snaps[-1]["fields"].get("总市值"))

    daily_pnl = round(total_mv - yesterday_mv, 2) if yesterday_mv else 0.0
    ...
    row = {
        ...
        "总成本": total_cost,
        "总浮盈": total_profit,
        "总浮盈%": total_profit_pct,
        "当日盈亏": daily_pnl,
        "持仓只数": len(active),
    }
    bitable.batch_create(cfg.tables.asset_snapshots, [row])
